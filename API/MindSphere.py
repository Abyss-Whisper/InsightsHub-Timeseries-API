...
class MindSphere():
    ...
    def getAssetList(self):
        ...
        return json.loads(requests.get(f"{self.gateway_URL}api/assetmanagement/v3/assets?size=200",headers=headers).text)['_embedded']['assets']
    
    #Retorna a lista de Types
    def getTypeList(self):
        headers = {
            "Authorization":'bearer '+self.getBearerToken(),
            "Content-Type":"'application/json'",
        }
        return json.loads(requests.get(f"{self.gateway_URL}api/assetmanagement/v3/assettypes?size=200",headers=headers).text)['_embedded']['name']
